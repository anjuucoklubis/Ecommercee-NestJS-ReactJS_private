...
describe('ContactController', () => {
  ...
  describe('GET /api/contacts/get/:contactId', () => {
    beforeEach(async () => {
      await testService.deleteAll();
    ...
    it('should be rejected if contact is not found', async () => {
      ...
      const response = await request(app.getHttpServer())
        .get(`/api/contacts/get/${contact.id + 1}`)
        .set('Authorization', 'test');
  ...
  describe('PUT /api/contacts/update/:contactId', () => {
    beforeEach(async () => {
      await testService.deleteAll();
    ...
    it('should be rejected if contact is not found', async () => {
      const contact = await testService.getContact();
      const response = await request(app.getHttpServer())
        .put(`/api/contacts/update/${contact.id + 1}`)
        .set('Authorization', 'test')
        .send({
          first_name: 'test',
          last_name: 'test',
  ...
  describe('DELETE /api/contacts/delete/:contactId', () => {
    ...
    it('should be rejected if contact is not found', async () => {
      const contact = await testService.getContact();
      const response = await request(app.getHttpServer())
        .delete(`/api/contacts/delete/${contact.id + 1}`)
        .set('Authorization', 'test');
